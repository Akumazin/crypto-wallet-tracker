    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 p-4 rounded-2xl bg-dark-900 border border-slate-800">
        ...
        <div className="flex items-center space-x-1.5 overflow-x-auto">
          <span className="text-xs text-slate-500 font-mono hidden sm:inline mr-1">Filtro:</span>
          {[
            { id: 'all', label: 'Todas' },
            { id: 'NFT', label: 'NFTs (Mints/Trades)' },
        ...
        {filteredTransactions.length === 0 ? (
          <div className="text-center py-16 px-4 rounded-2xl bg-dark-900/60 border border-slate-800">
            ...
            <p className="text-xs text-slate-400 max-w-md mx-auto mt-1">
              Tente alterar os filtros de busca ou aguarde novos blocos nas 9 redes monitoradas.
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                  ...
                  <div className="flex items-start sm:items-center space-x-3.5">
                    ...
                    {tx.nftImage ? (
                      <div className="relative w-12 h-12 rounded-xl overflow-hidden border border-purple-500/40 shrink-0 bg-dark-950">
                        <img 
                          src={tx.nftImage} 
                      ...
                      <div className="mt-1.5 text-xs text-slate-300 flex flex-wrap items-center gap-2">
                        {tx.type.startsWith('NFT') ? (
                          <span className="font-semibold text-purple-300">
                            {tx.nftCollection} <span className="text-slate-400 font-mono">{tx.nftTokenId}</span>
                          </span>
                        ) : tx.type === 'TOKEN_SWAP' ? (
                  ...
                  {/* Right Column: Values, Gas, Hash, Timestamp */}
                  <div className="flex items-center justify-between lg:justify-end w-full lg:w-auto gap-6 pt-2 lg:pt-0 border-t lg:border-t-0 border-slate-800/60">
                    {/* Value in USD */}
